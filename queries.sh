...
echo "$($PSQL "SELECT t.name FROM games g JOIN teams t ON t.team_id = g.winner_id WHERE round = 'Final' AND YEAR = 2018")"
...
echo "$($PSQL "SELECT g.year, t.name FROM games g JOIN teams t ON t.team_id = g.winner_id WHERE g.round = 'Final' ORDER BY year")"
